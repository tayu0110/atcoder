...
seq 0 99 |
  ...
  xargs -I@ printf "./tools/target/debug/tester ./target/debug/ahc25 < ./tools/in/%04d.txt > ./tools/out/%04d.txt\n" @ @ |
  xargs -L1 -n1 -P"$(nproc)" -I@ bash -c @
seq 0 99 |
  xargs -I@ printf "tools/target/debug/vis tools/in/%04d.txt tools/out/%04d.txt\n" @ @ |
