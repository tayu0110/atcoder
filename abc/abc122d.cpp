...
int main(int argc,char* argv[]){
  ...
  for(int i=0;i<4;i++) for(int j=0;j<4;j++) for(int k=0;k<4;k++) dp[i][j][k][3] = 1;
  dp[A][G][C][3] = 0;
  dp[A][C][G][3] = 0;
  dp[G][A][C][3] = 0;
  for(int l=3;l<n;l++) {
    for(int i=0;i<4;i++) for(int j=0;j<4;j++) for(int k=0;k<4;k++) {
      for(int m=0;m<4;m++) {
        ...
        dp[j][k][m][l+1] += dp[i][j][k][l];
      }
    }
  }
  mint ans = 0;
  for(int i=0;i<4;i++) for(int j=0;j<4;j++) for(int k=0;k<4;k++) ans += dp[i][j][k][n];
  cout << ans << endl;
  return 0;
}
