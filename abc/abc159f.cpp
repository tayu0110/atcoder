...
int main(int argc, char* argv[]){
  ...
  const int mx = s + 1;
  vector<vector<vector<mint>>> dp(n+10, vector<vector<mint>>(6010, vector<mint>(3)));
  dp[0][0][0] = 1;
  for(int i=0;i<n;i++) for(int j=0;j<mx;j++) {
    for(int k=0;k<3;k++) for(int l=k;l<3;l++) {
      dp[i+1][j][l] += dp[i][j][k];
      if(l && k != 2) dp[i+1][j+a[i]][l] += dp[i][j][k];
    }
  }
  cout << dp[n][s][2] << endl;
  return 0;
