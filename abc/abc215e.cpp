...
int main(int argc,char* argv[]){
  ...
  cin >> s;
  set<char> ck;
  for(int i=0;i<n;i++) ck.insert(s[i]);
  string c;
  for(auto e : ck) c += e;
  vector<vector<mint>> dp(1<<c.size(), vector<mint>(c.size(), 0));
  ck.clear();
  for(int i=0;i<n;i++) {
    char nc = s[i];
    int m = 0;
    vector<vector<mint>> tmp = dp;
    while(c[m] != nc) m++;
    for(int b=0;b<(1<<c.size());b++) {
      tmp[b][m] += dp[b][m];
      if(!(b & (1<<m))) {
        int t = b | (1<<m);
  ...
  mint ans = 0;
  for(int i=0;i<(1<<c.size());i++) for(int j=0;j<c.size();j++) ans += dp[i][j];
  cout << ans << endl;
  return 0;
}
