...
vector<int> l;
bool dfs(int now, Graph &t, set<int> &ck) {
  if(now == t.size()-1) {
    l.push_back(now);
    return true;
  }
  ...
  for(int i=0;i<t[now].size();i++) {
    if(ck.find(t[now][i]) != ck.end()) continue;
    bool f = dfs(t[now][i], t, ck);
    if(f) {
      l.push_back(now);
      return true;
    }
...
int dfs2(int now, int r, Graph &t, set<int> &ck) {
  if(now == r) return 0;
  ck.insert(now);
  int res = 1;
  for(int i=0;i<t[now].size();i++) {
    if(ck.find(t[now][i]) != ck.end()) continue;
    res += dfs2(t[now][i], r, t, ck);
  }
  return res;
}
...
int main(int argc,char* argv[]){
  ...
  // cout << endl;
  set<int> ck2;
  int bn = dfs2(0, b, t, ck2);
  int wn = dfs2(n-1, w, t, ck2);
  // cout << "b: " << b << " w: " << w << endl;
  // cout << "bn: " << bn << " wn: " << wn << endl; 
  if(bn > wn) cout << "Fennec" << endl;
