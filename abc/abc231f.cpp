...
int main(int argc, char* argv[]){
  ...
  for(int i=0;i<n;i++) cin >> b[i], mpb[b[i]] = 0;
  vector<pll> p(n);
  for(int i=0;i<n;i++) p[i] = {a[i], b[i]};
  sort(p.begin(), p.end(), [](pll lhs, pll rhs) {
  ...
  ll ans = 0;
  int cnt = 1;
  for(auto &[f, s] : mpb) s = cnt++;
  for(int i=0;i<n;i++) b[i] = mpb[b[i]];
  ll add = 1;
  for(int i=0;i<n;i++) {
    if(i < n-1 && b[i+1] == b[i] && a[i+1] == a[i]) {
      add++;
      continue;
    }
    ft.add(b[i], add);
    ans += add * ((i+1) - ft.getSum(0, b[i]));
    add = 1;
  }
  cout << ans << endl;
  return 0;
