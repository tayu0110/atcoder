...
mint dfs(ll k, int now, vector<ll>& a) {
  ll sum = 0;
  mint res = 0;
  int n = a.size();
  if(now == n) return 1;
  for(int i=now;i<n;i++) {
    sum += a[i];
    if(sum % k != 0) continue;
    if(ck[i+1][k+1]) res += memo[i+1][k+1];
    else res += dfs(k+1, i+1, a);
  }
  ck[now][k] = true;
  return memo[now][k] = res;
}
int main(int argc,char* argv[]){
  ...
  memo.assign(n+1, vector<mint>(n+2));
  ck.assign(n+1, vector<bool>(n+2, false));
  mint ans = dfs(1, 0, a);
  cout << ans << endl;
  return 0;
}
