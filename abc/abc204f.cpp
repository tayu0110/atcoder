...
int main(int argc, char* argv[]) {
  ...
  if(w == 1) {
    cout << accumulate(mask.begin(), mask.end(), mint(0)) << endl;
    return 0;
  }
  vector<vector<mint>> dp(1 << h, vector<mint>(1 << h, 0));
  for(int i=0;i<(1<<h);i++) {
    for(int j=0;j<(1<<h);j++) {
      for(int k=0;k<(1<<h);k++) {
        if(i & k) continue;
  ...
  Matrix<mint> m(dp);
  m = m.pow(w-1);
  vector<vector<mint>> v(1 << h, vector<mint>(1, 0));
  for(int i=0;i<(1<<h);i++) v[i][0] = mask[i];
  m *= Matrix<mint>(v);
  mint ans = 0;
  DEBUG(m.row);DEBUG_EN(m.column);
  for(int i=0;i<(1<<h);i++) {
    ans += m.get(i, 0);
  }
  cout << ans << endl;
