...
int main(int argc,char* argv[]){
  ...
  sort(p.begin(), p.end());
  if(tmp != 1) cnt++;
  vector<vector<mint>> dp(cnt+1, vector<mint>(p.size()));
  dp[0][0] = 1;
  for(int i=0;i<cnt;i++) {
    for(auto e : p) {
      if(e == 1) continue;
      for(int j=0;j<p.size();j++) {
        if(p[j] * e > m) break;
        int idx = lower_bound(p.begin(), p.end(), p[j] * e) - p.begin();
        if(p[idx] != p[j] * e) continue;
        dp[i+1][idx] += dp[i][j];
      }
    }
  }
  combination c(n);
  mint ans = 0;
  for(int i=1;i<min(n+1, cnt+1);i++) ans += c(n, n-i) * dp[i][p.size()-1];
  cout << ans << endl;
  return 0;
}
