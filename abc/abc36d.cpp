...
mint dfs(int now, int col, int par, Graph &t) {
  if(ck[now][col]) return memo[now][col];
  ck[now][col] = true;
  mint res = 1;
  for(auto e : t[now]) {
    if(e == par) continue;
    mint tmp = dfs(e, 0, now, t);
    if(col != 1) tmp += dfs(e, 1, now, t);
    res *= tmp;
  }
  return memo[now][col] = res;
}
int main(int argc,char* argv[]){
  ...
  memo.assign(n, vector<mint>(2, 0));
  ck.assign(n, vector<bool>(2, false));
  mint ans = dfs(0, 0, -1, t) + dfs(0, 1, -1, t);
  cout << ans << endl;
  return 0;
}
