...
ll a, b, k;
vector<vector<ll>> dp;
string rec(ll nk, int an, int bn) {
  if(an == 0) {
    string s = "";
    for(int i=0;i<bn;i++) s+= "b";
    return s;
  }
  if(bn == 0) {
    string s = "";
    for(int i=0;i<an;i++) s+= "a";
    return s;
  }
  if(nk <= dp[an-1][bn]) return "a" + rec(nk, an-1, bn);
  else return "b" + rec(nk-dp[an-1][bn], an, bn-1);
}
