...
int main(int argc,char* argv[]){
  ...
  vector<vector<mint>> dp(2, vector<mint>(n-1, 0LL));
  dp[0][0] = dp[1][0] = 1;
  for(int i=1;i<n-1;i++) {
    dp[0][i] = dp[0][i-1] + dp[1][i-1];
    dp[1][i] = dp[0][i-1];
  }
  vector<vector<mint>> dp2(2, vector<mint>(n-1, 0LL));
  dp2[0][n-2] = dp[0][n-2]; dp2[1][n-2] = dp[1][n-2];
  for(int i=n-3;i>=0;i--) {
    dp2[0][i] = dp2[1][i+1] + dp2[0][i+1] / (dp[0][i] + dp[1][i]) * dp[0][i];
    dp2[1][i] = dp2[0][i+1] / (dp[0][i] + dp[1][i]) * dp[1][i];
  }
  mint ans = 0;
  for(int i=n-1;i>0;i--) {
    ans += a[i] * (dp2[0][i-1] - dp2[1][i-1]);
  }
  ans += a[0] * (dp[0][n-2] + dp[1][n-2]);
  cout << ans << endl;
  return 0;
}
