...
fn main() {
    ...
    let mut st = segtree::SegmentTree::new(n, 0, |l, r| std::cmp::max(l, r));
    let mut sum = 0;
    ...
    let mut res = std::cmp::min(sum, l * n as i64);
    for i in 0..n {
        let t = st.get(i, n);
        res = std::cmp::min(res, sum - t);
        sum -= a[i] - l;
        // println!("t: {}, res: {}, sum: {}", t, res, sum);
        let t = st.get(i+1, i+2);
        st.update(i, t + (l - r));
    }
