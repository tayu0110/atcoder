...
mint rec(int now, int par, int pc, Graph &t) {
  if(ck[pc][now]) return memo[pc][now];
  mint res = 1;
  for(auto e : t[now]) {
    if(e == par) continue;
    mint tmp = rec(e, now, 0, t);
    if(!pc) tmp += rec(e, now, 1, t);
    res *= tmp;
  }
  ck[pc][now] = true;
  return memo[pc][now] = res;
}
int main(int argc, char* argv[]){
  ...
  memo.assign(2, vector<mint>(n, 0));
  ck.assign(2, vector<bool>(n, false));
  mint ans = rec(0, -1, 0, t);
  ans += rec(0, -1, 1, t);
  cout << ans << endl;
  return 0;
}
