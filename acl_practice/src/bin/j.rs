...
fn main() {
    ...
    let mut st = SegmentTree::new(n, 0, std::cmp::max);
    for (i, v) in a.into_iter().enumerate() {
        st.update(i, v);
    }

    for _ in 0..q {
        input! {t: usize, x: i64, v: i64};
        if t == 1 {
            st.update(x as usize - 1, v);
        } else if t == 2 {
            let (l, r) = (x-1, v);
            println!("{}", st.get(l as usize, r as usize));
        } else {
            let (mut l, mut r) = (x-1, n as i64);
            while r - l > 1 {
                let m = (r + l) / 2;
                if st.get(l as usize, m as usize) >= v {
                    r = m;
                } else {
                    l = m;
                }
            ...
            if st.get(l as usize, r as usize) >= v {
                println!("{}", l+1);
            } else {
                println!("{}", n+1);
            }
