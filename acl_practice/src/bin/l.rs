...
mod segtree {
    ...
    pub fn range_add_range_sum_query(size: usize) -> LazySegtree<(i64, i64), i64> {
        LazySegtree::from_vec(
            ...
            || (0, 0),
            || 0i64,
            |f, x| if f == 1 { (x.1 - x.0, x.1) } else { (x.0, x.1) },
            |f, g| (f + g) % 2)
    }
    #[allow(dead_code)]
...
fn main() {
    input! {n: usize, q: usize, a: [usize; n], p: [(usize, usize, usize); q]};
    let a = a.into_iter().map(|v| (1 - v, v, 0usize)).collect();

    let op = |l: (usize, usize, usize), r: (usize, usize, usize)| (l.0+r.0, l.1+r.1, l.2+r.2+l.1*r.0);
    let e = || (0, 0, 0);
    let mapping = |l: bool, r: (usize, usize, usize)| if l { (r.1, r.0, r.0 * r.1 - r.2) } else { r };
