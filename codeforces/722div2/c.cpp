...
vector<vector<ll>> dp;
void dfs(int from, int now, Graph& t, vector<pll>& g) {
  if(t[now].size() == 1 && t[now][0] == from) {
    ll pl = g[from].first;
    ll pr = g[from].second;
    ll l = g[now].first;
    ll r = g[now].second;
    dp[0][from] += max(abs(pl - l), abs(pl - r));
    dp[1][from] += max(abs(pr - l), abs(pr - r));
    return;
  }
  ...
  if(from != -1) {
    ...
    ll l = g[now].first;
    ll r = g[now].second;
    dp[0][from] += max(abs(pl - l) + dp[0][now], abs(pl - r) + dp[1][now]);
    dp[1][from] += max(abs(pr - l) + dp[0][now], abs(pr - r) + dp[1][now]);
  }
  return;
}
...
int main(int argc,char* argv[]){
  ...
  while(T--) {
    ...
    dp.assign(2, vector<ll>(n));
    dfs(-1, 0, t, g);
    cout << max(dp[0][0], dp[1][0]) << endl;
    // res.push_back(max(dp[0][0], dp[1][0]));
  }
