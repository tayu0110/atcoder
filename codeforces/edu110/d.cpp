...
void init() {
  ...
  for(int i=s.length()-1;i>=0;i--) {
    if(s[i] == '?') {
      if(i*2+2 >= res.size()) res[i] = 2;
      else res[i] = res[i*2+1] + res[i*2+2];
    } else {
      if(i*2+2 >= res.size()) res[i] = 1;
      else {
        if(s[i] == '0') res[i] = res[i*2+2];
        else res[i] = res[i*2+1];
      }
    }
  }
}
ll update(int idx, char c) {
  idx = s.length() - idx;
  s[idx] = c;
  while(idx >= 0) {
    if(c == '?') {
      if(idx*2+2 >= res.size()) res[idx] = 2;
      else res[idx] = res[idx*2+1] + res[idx*2+2];
    } else {
      if(idx*2+2 >= res.size()) res[idx] = 1;
      else {
        if(s[idx] == '0') res[idx] = res[idx*2+2];
        else res[idx] = res[idx*2+1];
      }
    }
    if(idx == 0) break;
    idx = (idx+1) / 2 - 1;
