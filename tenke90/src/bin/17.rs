...
fn main() {
    ...
    // let size = n + 2;
    // let mut st = LazySegtree::from_vec(&vec![(0i64, 1i64); size], |l, r| (l.0+r.0, l.1+r.1), || (0, 0), || 0i64, |f, x| (x.0+f*x.1, x.1), |f, g| f + g);
    let mut st = segtree::range_add_range_sum_query(n+2);
    let mut t = vec![vec![]; n+1];
    for &(l, r) in &p {
        st.apply_range(l+1, r, 1);
        t[l].push(r);
    }
