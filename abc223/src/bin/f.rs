...
fn main() {
    input! {n: usize, q: usize, mut s: Chars, p: [(usize, usize, usize); q]}
    ...
    let mut range_sum_range_minimum = {
        ...
        segtree::LazySegtree::from_vec(&vec![0; n], op, e, id, mapping, composition)
    };

    for (i, &c) in s.iter().enumerate() {
        if c == '(' {
            range_sum.update(i, 1);
            range_sum_range_minimum.apply_range(i, n, 1);
        } else {
            range_sum.update(i, -1);
            range_sum_range_minimum.apply_range(i, n, -1);
        }
    ...
    for (t, l, r) in p {
        if t == 1 {
            let (lc, rc) = (s[l-1], s[r-1]);
            if lc == rc {
                continue;
            }
            if lc == '(' {
                range_sum.update(l-1, -1);
                range_sum.update(r-1, 1);
                range_sum_range_minimum.apply_range(l-1, r-1, -2);
            } else {
                range_sum.update(l-1, 1);
                range_sum.update(r-1, -1);
                range_sum_range_minimum.apply_range(l-1, r-1, 2);
            ...
            let v = range_sum.get(l-1, r);
            if v != 0 {
                println!("No");
                continue;
            }
            let w = range_sum.get(0, l-1);
            let min = range_sum_range_minimum.prod(l-1, r);
            if min - w < 0 {
                println!("No");
            } else {
                println!("Yes");
