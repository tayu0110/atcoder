...
fn main() {
    ...
    let s = t.chars().chain(t.chars()).chain((0..2*n).map(|_| 'z')).chain(s.chars()).chain(s.chars()).collect::<String>();
    ...
    let mut res = 0;
    let mut ts = 0;
    for index in sa.into_iter().filter(|index| **index < n || (4*n <= **index && **index < 5*n)) {
        if *index < n {
            ts += 1;
        } else {
            res += ts;
