...
fn dfs(now: usize, in_cycle: &mut Vec<bool>, checked: &mut Vec<bool>, graph: &graph::DirectedGraph<graph::UnWeighted>) -> bool {
    if checked[now] {
        return in_cycle[now];
    }
    ...
    for to in graph.neighbors(now) {
        res |= dfs(*to, in_cycle, checked, graph);
    }
...
fn main() {
    ...
    let mut graph = graph::DirectedGraph::<graph::UnWeighted>::new(n);
    for (u, v) in p {
        graph.set_edge(u-1, v-1);
    }

    let t = graph::scc(&graph);
    ...
    for i in 0..n {
        ...
        dfs(i, &mut res, &mut ck, &graph);
    }

    println!("{}", res.iter().filter(|v| **v).count());
}
