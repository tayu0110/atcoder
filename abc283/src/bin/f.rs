...
fn main() {
    ...
    let (s, t) = {
        ...
        for (i, &p) in p.iter().enumerate() {
            res_inc.push(p + i as i32 - inc.get(0, p as usize + 1));
            res_dec.push(i as i32 - p + dec.get(p as usize + 1, n));

            inc.update(p as usize, p + i as i32);
            dec.update(p as usize, p - i as i32);
        }
    ...
    let (u, v) = {
        ...
        for (i, &p) in p.iter().enumerate().rev() {
            res_inc.push(-(p + i as i32) + (inc.get(p as usize, n)));
            res_dec.push(p - i as i32 - dec.get(0, p as usize + 1));
    ...
    println!(
        "{}",
        s.into_iter()
            .zip(t.into_iter().zip(u.into_iter().rev().zip(v.into_iter().rev())))
            .map(|(s, (t, (u, v)))| std::cmp::min(s, std::cmp::min(t, std::cmp::min(u, v))))
            .join(" ")
    )
}
