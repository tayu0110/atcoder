...
int main(int argc,char* argv[]){
  ...
  ans[0][0] = mint(3).pow(h*w-k);
  auto gor = [&](int y, int x) {
    if(x+1 < w) ans[y][x+1] += ans[y][x];
  };
  auto god = [&](int y, int x) {
    if(y+1 < h) ans[y+1][x] += ans[y][x];
  };
  mint inv = mint(3).inv();
  for(int i=0;i<h;i++) for(int j=0;j<w;j++) {
    if(t[i][j] == 'R') {
      ...
      if(i+1 < h) ans[i+1][j] += (ans[i][j] + ans[i][j]) * inv;
      if(j+1 < w) ans[i][j+1] += (ans[i][j] + ans[i][j]) * inv;
    }
  }
