...
mod segtree {
    ...
    where T: Clone + Copy + Sized + std::ops::AddAssign {
...
fn main() {
    ...
    let mut res = 0usize;
    let mut st = SegmentTree::new(n+1, 0, |l, r| l + r);
    let mut color = vec![vec![]; n+1];
    for (nc, nx) in c.into_iter().zip(x.into_iter()) {
        let r = st.get(nx+1, n+1);
        res += r;
        st.update(nx, 1);
        color[nc].push(nx);
    }
    ...
    for v in color {
        ...
        let mut st = SegmentTree::new(cnt, 0, |l, r| l + r);
        for w in v {
            let tmp = *map.get(&w).unwrap();
            res -= st.get(tmp+1, cnt);
            st.update(tmp, 1);
        }
    }
