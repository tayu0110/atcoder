...
fn main() {
    input! {n: usize, q: usize, x: usize, mut p: [usize; n], q: [(usize, usize, usize); q]}
    ...
    let p = p.into_iter().map(|c| (c, 1)).collect::<Vec<(_, _)>>();

    let op = |l: (usize, usize), r: (usize, usize)| (l.0 + r.0, l.1 + r.1);
    let e = || (0, 0);
    let id = || (1, 0);
    let mapping = |f: (usize, usize), x: (usize, usize)| if f.1 == 0 { (x.0, x.1) } else { (x.1 * f.0, x.1) };
    let composition = |f: (usize, usize), g: (usize, usize)| if f.1 > g.1 { f } else { g };
    let mut st = segtree::LazySegtree::from_vec(&p, op, e, id, mapping, composition);

    for (i, (c, l, r)) in q.into_iter().enumerate() {
        let (large, _) = st.prod(l-1, r);
        let small = r-l+1 - large;
        if c == 1 {
            st.apply_range(l-1, l+small-1, (0, i+1));
            st.apply_range(l+small-1, r, (1, i+1));
            if l-1 <= xi && xi < r {
                xi = l+small-2;
            }
        } else {
            st.apply_range(l-1, l+large-1, (1, i+1));
            st.apply_range(l+large-1, r, (0, i+1));
            if l-1 <= xi && xi < r {
                xi = l+large-1;
            }
        }
