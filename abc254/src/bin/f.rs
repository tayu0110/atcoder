...
fn gcd(mut l: i32, mut r: i32) -> i32 {
    if l < r {
...
fn main() {
    input! {n: usize, q: usize, a: [i32; n], b: [i32; n], p: [(usize, usize, usize, usize); q]};

    let mut sta = segtree::SegmentTree::new(n, 0, gcd);
    let mut stb = segtree::SegmentTree::new(n, 0, gcd);
    for i in 1..n {
        sta.update(i-1, (a[i] - a[i-1]).abs());
        stb.update(i-1, (b[i] - b[i-1]).abs());
    }

    for (w, x, y, z) in p {
        let tmp = gcd(sta.get(w-1, x-1), stb.get(y-1, z-1));
        println!("{}", gcd(tmp, a[w-1] + b[y-1]));
    }
}